...
| 2 | `fetch-polygons` | `config/buildings.json` | Fetches building footprints for the same `bbox` — used as map context, and as the input for step 5's impact zone |
...
### Isochrone from a single park

By default (`select_id: ""` in `accessibility.json`), `compute-accessibility`
pools every park in `polygons_geojsonfile` together, measuring "time to the
*nearest* park" across all of them at once. To compute the isochrone from
**one specific park** instead, set `select_id` to a value from that park's
`id_column` (`poly_id`, generated by `fetch-polygons`) in that same file —
no separate config needed. Look up a real `poly_id` by running
`fetch-polygons` on `parks.json` first and inspecting the resulting
`parks.geojson`. Leaving `select_id` empty (or unset) keeps the original
pooled, multi-park behavior; re-running with a value set overwrites the
same output files with that park's isochrone instead.

`visualize.json`'s `highlight_id` field mirrors this: set it to the same
`poly_id` to pick that park out from the rest on the map (bright orange
fill/outline, added to the legend as "Selected park"); leave it empty for
the plain multi-park rendering.
...
- `config/accessibility.json` — the pre-existing `geodecision.accessibility.accessibility.run`, see `geodecision/geodecision/accessibility/schema.py`.
  ...
  [Isochrone from a single park](#isochrone-from-a-single-park).
